...
weight: 7
date: 2026-06-05
tags: ["Python", "Django", "Web框架", "Ajax"]
---

Ajax（异步 JavaScript 和 XML）允许页面在不刷新的情况下与服务器交换数据，实现局部刷新效果。本文介绍在 Django 项目中使用 Ajax 发送 GET/POST 请求、传输 JSON 数据、上传文件，以及处理 CSRF 验证的完整方案。
